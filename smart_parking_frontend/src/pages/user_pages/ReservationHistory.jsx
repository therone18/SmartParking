...
const MyReservations = () => {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        setReservations(response.data); // Store all reservations
      } catch (error) {
  ...
  // Determine if reservation is future & cancelable
  const canBeCancelled = (res) => {
    return res.status === "Reserved" && dayjs(res.start_time).isAfter(dayjs());
  };

  // Check-out available only if status is Active
            ...
            {filteredReservations.map((res) => (
              ...
              >
                <div className="flex flex-col gap-2 text-slate-900">
                  <p><span className="text-sm text-gray-500">Reservation ID:</span> <span className="font-medium">{res.id}</span></p>
                  <p><span className="text-sm text-gray-500">Slot ID:</span> <span className="font-medium">{res.slot_id || res.slot || "N/A"}</span></p>
                  <p><span className="text-sm text-gray-500">Vehicle:</span> {res.vehicle_make} {res.vehicle_model}</p>
                  <p><span className="text-sm text-gray-500">Plate:</span> {res.plate_number}</p>
                  <p><span className="text-sm text-gray-500">Start Time:</span> {dayjs(res.start_time).format("YYYY-MM-DD HH:mm")}</p>
                  <p><span className="text-sm text-gray-500">End Time:</span> {dayjs(res.end_time).format("YYYY-MM-DD HH:mm")}</p>
                  <p>
